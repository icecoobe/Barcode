...
using Barcode._2D.QRCode.Codec.Util;
...
namespace Barcode._2D
{
    class QRCodeEncoder
    {
		...
		public virtual bool[][] calQrcode(byte[] qrcodeData)
		{
            ...
            uint ec; // NOTE: (icecoobe)参见formatInformationValue定义处
           ...
           //     MemoryStream memoryStream = new MemoryStream(Resources.GetResource(fileName));
            ...
            for (sbyte i = 1; i <= 127; i++) // NOTE: (icecoobe) 消除编译警告
			{
            ...
            // NOTE: 
            //  (icecoobe):
            //  在移位操作前, 应该转换成无符号数，ec只作为数组下标用，故暂时先将定义改成无符号数，无影响
            //  另，将其转换成字节更小的byte
            //  maskNumber转换成byte，也没做测试
            byte ec_smalltype = (byte)ec;
            sbyte formatInformationValue = (sbyte)(ec_smalltype << 3 | (byte)maskNumber); 
